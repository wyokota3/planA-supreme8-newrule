...
モデル解決の優先順位(copilot): `HARNESS_AUDIT_COPILOT_MODEL` > `HARNESS_AUDIT_MODEL` > 未指定(=`--model` を付けない=利用者の既定モデル)。

## exit code(呼び出し側の契約)

| code | 意味 | 呼び出し側の動作 |
| --- | --- | --- |
| `0` | 外部provider(copilot)が監査レポートを生成した | `reports/audit-*.md` を提示 |
| `10` | DELEGATE。既存 `auditor` サブエージェントで実行すべき | サブエージェントを起動(従来フロー) |
| `2` | 指定provider不可 & フォールバック不可(明示copilot or strict) | エラーをそのまま提示して停止 |
| `3` | 未実装provider(codex)が明示選択された | エラーを提示して停止 |
| `1` | 使い方/想定外エラー | エラーを提示 |
...
- **auto** — copilot が使えれば copilot、ダメなら existing へフォールバック(exit 10)。`HARNESS_AUDIT_STRICT_PROVIDER=1` のときはフォールバックせず fail(exit 2)。
- **codex** — 将来枠(BYOK / OpenAI互換)。このテンプレートでは未実装で、明示選択時は fail(exit 3)。
...
copilot provider は監査役にファイルを書かせない。copilot にはレポートを **stdout に出力**させ、
ファイル書き込みは `run-audit.ps1` が行う。監査provider側はリポジトリのファイルを変更しない。
